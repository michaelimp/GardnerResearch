...
def pasta_measurements(rho, proportion_idle_servers, dispatchers: List[Dispatcher]):
    ...
    for i in range(s):
        new_data = [pd.DataFrame({'idle_proportion': [total_idle_per_class[i] / NUM_SERVERS_PER_CLASS[i]]})]
        proportion_idle_servers[i] = pd.concat(new_data, ignore_index=True)
...
def main():
    ...
    rho = [0]
    proportion_idle_servers = [pd.DataFrame(columns=['idle_proportion'])] * s
    ...
    sns.histplot(proportion_idle_servers[0]['idle_proportion'], kde=True)
